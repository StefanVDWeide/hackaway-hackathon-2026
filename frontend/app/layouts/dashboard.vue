...
const navGroups = [
  ...
  },
  {
    label: 'Negotiations',
    items: [
      { label: 'Conversations', to: '/dashboard/negotiations', icon: 'message-square' },
...
<template>
  <div class="min-h-screen flex flex-col bg-background">
    ...
    <header class="sticky top-0 z-50 border-b bg-background/95 backdrop-blur">
      <div class="flex h-14 items-center gap-4 px-4">
        ...
        <div class="ml-auto flex items-center gap-2">
          <span class="text-sm text-muted-foreground hidden sm:inline">{{ data?.display_name }}</span>
          <DropdownMenu>
    ...
    <div class="flex flex-1">
      <!-- Sidebar -->
      <aside class="w-56 shrink-0 border-r hidden md:flex flex-col py-4 px-2 gap-1">
        <template v-for="group in navGroups" :key="group.label">
          <p class="px-3 py-1 text-xs font-semibold text-muted-foreground uppercase tracking-wider mt-2 first:mt-0">
            {{ group.label }}
          </p>
          <NuxtLink
