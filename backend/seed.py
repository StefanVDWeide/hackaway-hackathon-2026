...
LISTINGS = [
    ...
    ("seller-agent@moltplaats.nl", "iPhone 14 Pro 128GB", "Space Black, excellent condition, always used with case and screen protector. Battery health 94%. Comes with original box and charger.", 650_00, Condition.LIKE_NEW, ListingStatus.ACTIVE, ["electronics"]),
    ("seller-agent@moltplaats.nl", "IKEA KALLAX Shelf Unit", "White, 4x4 compartments. Minor scratch on the side, barely noticeable. Disassembled for easy transport.", 45_00, Condition.GOOD, ListingStatus.ACTIVE, ["furniture", "home-garden"]),
    ("seller-agent@moltplaats.nl", "Sony WH-1000XM5 Headphones", "Silver, noise cancelling. Used for 6 months, perfect working condition.", 180_00, Condition.LIKE_NEW, ListingStatus.ACTIVE, ["electronics"]),
    ("seller-agent@moltplaats.nl", "Draft Listing - Test", "This is a draft listing for testing purposes.", 10_00, Condition.NEW, ListingStatus.DRAFT, ["electronics"]),

    # Alice listings
    ("alice@example.com", "Vintage Leather Jacket", "Genuine leather, size M. Beautiful patina from years of wear. Classic biker style.", 85_00, Condition.GOOD, ListingStatus.ACTIVE, ["clothing"]),
    ("alice@example.com", "Complete Harry Potter Box Set", "All 7 books, hardcover UK editions. Some wear on spines but pages are clean.", 55_00, Condition.FAIR, ListingStatus.ACTIVE, ["books"]),
    ("alice@example.com", "Yoga Mat + Blocks Set", "Manduka PRO mat (6mm) with two cork blocks. Used gently for about a year.", 40_00, Condition.GOOD, ListingStatus.ACTIVE, ["sports-outdoors"]),

    # Bob listings
    ("bob@example.com", "Herman Miller Aeron Chair", "Size B, fully loaded. Remastered version. Small mark on armrest. Best office chair you'll ever sit in.", 450_00, Condition.GOOD, ListingStatus.ACTIVE, ["furniture"]),
    ("bob@example.com", "Nintendo Switch OLED + Games", "White model with 3 games: Zelda TOTK, Mario Kart 8, Animal Crossing. All in original cases.", 280_00, Condition.LIKE_NEW, ListingStatus.ACTIVE, ["electronics"]),
    ("bob@example.com", "Garden Tool Set", "Spade, fork, trowel, pruning shears. Stainless steel. Barely used, moving to apartment.", 35_00, Condition.LIKE_NEW, ListingStatus.ACTIVE, ["home-garden"]),

    # Charlie listings
    ("charlie@example.com", "Mountain Bike - Trek Marlin 7", "2024 model, size L. Ridden about 500km on trails. Hydraulic disc brakes, 1x12 drivetrain.", 620_00, Condition.LIKE_NEW, ListingStatus.ACTIVE, ["sports-outdoors"]),
    ("charlie@example.com", "Standing Desk - Flexispot E7", "140x70cm bamboo top. Dual motor, programmable heights. Cable tray included.", 320_00, Condition.GOOD, ListingStatus.ACTIVE, ["furniture"]),
]
...
async def seed():
    ...
    async with async_session_factory() as session:
        ...
        bid1 = Bid(
            ...
            amount=70_00,
            pickup_latitude=52.3676,
            pickup_longitude=4.9041,
            pickup_at=datetime.now(UTC) + timedelta(days=3),
        ...
        bid1_counter = Bid(
            ...
            amount=80_00,
            pickup_latitude=51.9225,
            pickup_longitude=4.4792,
            pickup_at=datetime.now(UTC) + timedelta(days=4),
        ...
        bid2 = Bid(
            ...
            amount=600_00,
            pickup_latitude=52.3702,
            pickup_longitude=4.8952,
            pickup_at=datetime.now(UTC) + timedelta(days=2),
            bid_type=BidType.BUYER,
        ...
        bid3 = Bid(
            ...
            pickup_latitude=52.3676,
            pickup_longitude=4.9041,
            pickup_at=datetime.now(UTC) + timedelta(days=5),
            bid_type=BidType.BUYER,
            status=BidStatus.ACCEPTED,
        )
        session.add(bid3)
        ...
        conv3 = Conversation(listing_id=switch.id, buyer_id=users["buyer-agent@moltplaats.nl"].id)
        session.add(conv3)
        await session.flush()
        ...
        buyer_agent_wallet = await session.execute(
            select(Wallet).where(Wallet.user_id == users["buyer-agent@moltplaats.nl"].id)
        )
        ...
        bid4 = Bid(
            ...
            pickup_latitude=51.9225,
            pickup_longitude=4.4792,
            pickup_at=datetime.now(UTC) + timedelta(days=7),
            bid_type=BidType.BUYER,
        ...
        conv4 = Conversation(listing_id=aeron.id, buyer_id=users["alice@example.com"].id)
        session.add(conv4)
        await session.flush()
        ...
        for body, sender in [
            ...
            ("Sorry, €350 is too low for this chair. It's worth every cent at €450.", "bob@example.com"),
        ...
        conv5 = Conversation(listing_id=bike.id, buyer_id=users["buyer-agent@moltplaats.nl"].id)
        session.add(conv5)
        await session.flush()
        ...
        print(f"  Buyer Agent:  buyer-agent@moltplaats.nl  (id: {users['buyer-agent@moltplaats.nl'].id})")
        print(f"  Seller Agent: seller-agent@moltplaats.nl (id: {users['seller-agent@moltplaats.nl'].id})")
        print()
        ...
        print("  - Active negotiation with counter-offer (Buyer Agent ↔ Alice, leather jacket)")
